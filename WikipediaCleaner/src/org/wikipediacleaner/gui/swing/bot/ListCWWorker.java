...
public class ListCWWorker extends BasicWorker {
  ...
  public ListCWWorker(EnumWikipedia wiki, BasicWindow window, File dumpFile,
                      ...
                      Collection<Integer> selectedNamespaces, boolean checkWiki,
                      boolean onlyRecheck) {
    super(wiki, window);
    this.dumpFile = dumpFile;
    this.output = null;
  ...
  public Object construct() {
    ...
    CWPageProcessor pageProcessor =
        new CWPageProcessor(getWikipedia(), this, selectedNamespaces);
    if (onlyRecheck) {
      try {
  ...
  private String generateResult(List<Detection> pages, Long maxSize) {
    ...
    for (Detection detection : pages) {
      ...
      line.append(PageElementInternalLink.createInternalLink(
          Namespace.isColonNeeded(detection.namespace), detection.pageName,
          null));
   ...
   */
  private void outputResultToPage(CheckErrorAlgorithm algorithm,
                                  List<Detection> pages, String outputPage)
      throws APIException {
    ...
    Page page =
        DataManager.getPage(getWikipedia(), truePageName, null, null, null);
    API api = APIFactory.getAPI();
    ...
    while (attemptCount < 10) {
      ...
      if ((begin < 0) || (end < 0) || (end < begin)) {
        throw new APIException("Page " + truePageName +
                               " doesn't have place holders for the result");
      }
      ...
      do {
        ...
        newText.append("\n");
        newText.append(generateResult(tmpPages, maxSize - begin -
                                                    contents.length() + end));
        ...
        if (!text.equals(contents)) {
          api.updatePage(getWikipedia(), page, text,
                         "Dump analysis for error n°" +
                             algorithm.getErrorNumberString() + " (" + nbPages +
                             " pages)",
                         true, true, false);
        }
        return;
   ...
   */
  private void outputResultToFile(CheckErrorAlgorithm algorithm,
                                  List<Detection> pages, File outputPath) {
