...
/**
 * Class containing information about a complete external link ([http://...
 * text]).
 */
public class PageElementExternalLink extends PageElement {
  ...
  private final static String OUTSIDE_TEMPLATE_SEPARATORS =
      SEPARATORS_EXCLUDED + SEPARATORS_INCLUDED + UNACCEPTABLE + "[]";

  private final static String IN_TEMPLATES_SEPARATORS =
      OUTSIDE_TEMPLATE_SEPARATORS + "|}";
   ...
   */
  public static PageElementExternalLink analyzeBlock(EnumWikipedia wikipedia,
    ...
    if (hasSquare) {
      while ((tmpIndex < maxLength) && (contents.charAt(tmpIndex) == ' ')) {
        tmpIndex++;
      }
    }
    ...
    if (!hasSquare || (endUrlIndex >= maxLength) ||
        (UNACCEPTABLE.indexOf(contents.charAt(endUrlIndex)) >= 0)) {
      return new PageElementExternalLink(
          beginUrlIndex, endUrlIndex,
          contents.substring(beginUrlIndex, endUrlIndex), beginUrlIndex,
          endUrlIndex, null, -1, false, false);
    }
    if ((endUrlIndex < maxLength) && (contents.charAt(endUrlIndex) == ']')) {
      return new PageElementExternalLink(
          index, endUrlIndex + 1,
          contents.substring(beginUrlIndex, endUrlIndex), beginUrlIndex,
          endUrlIndex, null, -1, true, true);
    }
    ...
    if (analysis != null) {
      PageElementTag refTag =
          analysis.getSurroundingTag(PageElementTag.TAG_WIKI_REF, index);
    ...
    while (endTextIndex < maxEndIndex) {
      if (contents.startsWith("[[", endTextIndex)) {
        ...
        doubleSquareCount++;
        endTextIndex += 2;
      } else if ((doubleSquareCount > 0) &&
                 contents.startsWith("]]", endTextIndex)) {
        doubleSquareCount--;
        endTextIndex += 2;
        ...
        if (prematureEndIndex < 0) {
          return new PageElementExternalLink(
              ...
              contents.substring(beginUrlIndex, endUrlIndex), beginUrlIndex,
              endUrlIndex, contents.substring(beginTextIndex, endTextIndex),
              beginTextIndex - index, true, true);
        }
        return new PageElementExternalLink(
            index, prematureEndIndex,
            contents.substring(beginUrlIndex, endUrlIndex), beginUrlIndex,
            endUrlIndex, contents.substring(beginTextIndex, prematureEndIndex),
        ...
        if ((contents.charAt(endTextIndex) == '<') && (analysis != null)) {
          ...
          tagNowiki =
              analysis.isInTag(endTextIndex, PageElementTag.TAG_WIKI_NOWIKI);
        }
        if (comment != null) {
    ...
    for (String protocol : privateProtocols) {
      ...
      while (same && (pos < protocol.length())) {
        if (offset + pos >= text.length()) {
          same = false;
        } else if (protocol.charAt(pos) !=
                   Character.toLowerCase(text.charAt(offset + pos))) {
          same = false;
        } else {
          pos++;
        }
   ...
   */
  public int getLinkBeginIndex() { return linkBeginIndex; }

  /**
   * @return End index of the external link.
   */
  public int getLinkEndIndex() { return linkEndIndex; }
   ...
   * @return Text.
   */
  public String getTextNotTrimmed() { return textNotTrimmed; }
   ...
   */
  public boolean hasSquare() { return hasSquare; }

  /**
   * @return True if the link is in [...] and not in ([...[[...]]...])
   */
  public boolean hasSecondSquare() { return hasSecondSquare; }
   ...
   */
  private PageElementExternalLink(int beginIndex, int endIndex, String link,
                                  int linkBeginIndex, int linkEndIndex,
