...
public class Hc5HttpServer implements HttpServer {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(Hc5HttpServer.class);
    ...
    while (count < MAX_ATTEMPTS) {
      ...
      try {
        final HttpUriRequest method =
            Hc5HttpUtils.createMethod(url, properties, false);
        final Hc5HttpResponse response =
            httpClient.execute(method, new Hc5HttpResponseHandler());
  ...
  public void sendGet(String path, ResponseManager manager)
    ...
    while (count < MAX_ATTEMPTS) {
      ...
      try {
        final HttpUriRequest method =
            Hc5HttpUtils.createMethod(url, null, true);
        final Hc5HttpResponse response =
            httpClient.execute(method, new Hc5HttpResponseHandler());
        statusCode = response.status;
        if (statusCode == HttpStatus.SC_OK) {
