...
public class CheckErrorAlgorithm022 extends CheckErrorAlgorithmBase {
  ...
  public CheckErrorAlgorithm022() { super("Category with space"); }
  ...
  public boolean analyze(PageAnalysis analysis,
    ...
    Namespace categoryNamespace =
        analysis.getWikiConfiguration().getNamespace(Namespace.CATEGORY);
    ...
    for (PageElementCategory category : categories) {
      ...
      if (spaceFound) {
        ...
        String namespace =
            CharacterUtils.trim(category.getCategoryNotTrimmed());
        CheckErrorResult errorResult = createCheckErrorResult(
            analysis, category.getBeginIndex(), category.getEndIndex());
        if (sortSimple == null) {
          errorResult.addReplacement("[[" + namespace + ":" + nameSimple + "]]",
                                     true);
          if (!namespace.equals(preferredCategory)) {
            errorResult.addReplacement("[[" + preferredCategory + ":" +
                                       nameSimple + "]]");
          }
        } else {
          errorResult.addReplacement("[[" + namespace + ":" + nameSimple + "|" +
