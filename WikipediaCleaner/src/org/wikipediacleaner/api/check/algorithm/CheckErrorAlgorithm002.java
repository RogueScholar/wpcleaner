...
public class CheckErrorAlgorithm002 extends CheckErrorAlgorithmBase {
  ...
  private boolean analyzeNonFullTags(PageAnalysis analysis,
    ...
    if (PageElementTag.TAG_HTML_CITE.equals(tagName) ||
      ...
      String anchorProperty =
          getSpecificProperty("anchor_templates", true, true, false);
      if (anchorProperty != null) {
        anchorTemplates =
            WPCConfiguration.convertPropertyToStringArrayList(anchorProperty);
      }
    }
    ...
    for (PageElementTag tag : tags) {
      ...
      if (shouldReport) {
        ...
        if ((anchorTemplates != null) && !anchorTemplates.isEmpty() &&
  ...
  public Map<String, String> getParameters() {
    ...
    parameters.put("anchor_templates",
                   GT._T("A replacement for {0}", "&lt;span id=\"xxx\"/&gt;"));
    parameters.put("clear_all",
                   GT._T("A replacement for {0}", "&lt;br clear=\"all\"/&gt;"));
    parameters.put("clear_left", GT._T("A replacement for {0}",
                                       "&lt;br clear=\"left\"/&gt;"));
    parameters.put("clear_right", GT._T("A replacement for {0}",
                                        "&lt;br clear=\"right\"/&gt;"));
    return parameters;
  }
}
