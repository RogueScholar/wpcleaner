...
public class CheckErrorAlgorithm092 extends CheckErrorAlgorithmBase {
  ...
  public CheckErrorAlgorithm092() { super("Headline double"); }
  ...
  public boolean analyze(PageAnalysis analysis,
    ...
    for (PageElementTitle title : analysis.getTitles()) {
      ...
      if (titleLevel <= maxLevel) {
        HashMap<String, PageElementTitle> knownTitles =
            titles.get(Integer.valueOf(titleLevel));
        ...
        if (knownTitles == null) {
          ...
          if (previousTitle != null) {
            CheckErrorResult errorResult = createCheckErrorResult(
                analysis, previousTitle.getBeginIndex(),
                previousTitle.getEndIndex(), ErrorLevel.CORRECT);
  ...
  protected String internalAutomaticFix(PageAnalysis analysis) {
    ...
    if ((!analysis.getPage().isArticle()) ||
        (!analysis.getWPCConfiguration().isEncyclopedicNamespace(
            analysis.getPage().getNamespace())) ||
    ...
    for (int i = 1; i < titles.size(); i++) {
      ...
      if ((previousTitle.getLevel() <= maxLevel) &&
        ...
        String betweenTitles = contents
                                   .substring(previousTitle.getEndIndex(),
        ...
        if (shouldRemove) {
          if (previousTitle.getBeginIndex() > lastIndex) {
            buffer.append(
                contents.substring(lastIndex, previousTitle.getBeginIndex()));
          ...
          String afterTitle =
              contents
                  .substring(currentTitle.getEndIndex(),
                             (nextTitle != null) ? nextTitle.getBeginIndex()
                                                 : contents.length())
                  .trim();
          ...
          if (shouldRemove) {
            if (currentTitle.getBeginIndex() > lastIndex) {
              buffer.append(
                  contents.substring(lastIndex, currentTitle.getBeginIndex()));
              lastIndex = currentTitle.getBeginIndex();
            }
            lastIndex = (nextTitle != null) ? nextTitle.getBeginIndex()
                                            : contents.length();
          }
        }
      }
  ...
  public Map<String, String> getParameters() {
    ...
    parameters.put(PARAMETER_MAX_LEVEL,
                   GT._T("Maximum level of titles to report"));
    parameters.put(PARAMETER_ONLY_CONSECUTIVE,
