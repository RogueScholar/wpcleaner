...
public class CheckErrorAlgorithm067 extends CheckErrorAlgorithmBase {
  ...
  public CheckErrorAlgorithm067() { super("Reference after punctuation"); }
  ...
  public boolean analyze(PageAnalysis analysis,
    ...
    List<String> tmpGeneralAbbreviations =
        config.getStringList(WPCConfigurationStringList.ABBREVIATIONS);
    if ((tmpGeneralAbbreviations != null) &&
        (tmpGeneralAbbreviations.size() > 0)) {
    ...
    while (tagIndex < maxTags) {
      ...
      if (firstTag.isComplete() || !firstTag.isEndTag()) {
        for (int currentTagIndex = firstTagIndex;
             currentTagIndex <= lastTagIndex; currentTagIndex++) {
          PageElementTag currentTag = tags.get(currentTagIndex);
          if ((currentTagIndex == firstTagIndex) || (currentTag.isFullTag()) ||
              (!currentTag.isEndTag())) {
            ...
            while (!done) {
              ...
              if (testIndex >= 0) {
                ...
                if (Character.isWhitespace(testChar)) {
                  ...
                  if ((comment != null) &&
                      (comment.getEndIndex() == testIndex + 1)) {
                    if (currentTagIndex == firstTagIndex) {
                      previousComment += contents.substring(
            ...
            if (testIndex >= 0) {
              ...
              if (SpecialCharacters.isPunctuation(currentPunctuation)) {
                ...
                if (punctuation == ';') {
                  ...
                  testIndex--;
                  while ((testIndex >= 0) && (Character.isLetterOrDigit(
                                                 contents.charAt(testIndex)))) {
                    testIndex--;
                  }
                  if ((testIndex >= 0) && (contents.charAt(testIndex) == '&')) {
                    String name =
                        contents.substring(testIndex + 1, punctuationIndex);
                    for (HtmlCharacters htmlCharacter :
                         HtmlCharacters.values()) {
      ...
      if ((punctuationFoundBefore && (abbreviationsList != null))) {
        for (String abbreviation : abbreviationsList) {
          if (abbreviation != null) {
            if (contents.startsWith(abbreviation,
                                    tmpIndex - abbreviation.length() + 1)) {
      ...
      if ((punctuationFoundBefore && !abbreviationFound) ||
        ...
        if (punctuationFoundBefore && !abbreviationFound) {
          ...
          String allPunctuations =
              contents.substring(tmpIndex, lastPunctuationIndex + 1);
          ...
          String punctuationAfter =
              contents.substring(punctuationAfterIndex, tmpIndex);
          ...
          errorResult.addReplacement(prefix + previousComment + replace +
                                         moveablePrefix + allPunctuations,
                                     prefix + textReplace + moveablePrefix +
                                         allPunctuations,
                                     automatic);
          if (punctuationFoundAfter &&
              !allPunctuations.equals(punctuationAfter)) {
            errorResult.addReplacement(prefix + previousComment + replace +
                                           moveablePrefix + punctuationAfter,
                                       prefix + textReplace + moveablePrefix +
                                           punctuationAfter);
          }
  ...
  protected void initializeSettings() {
    String tmp =
        getSpecificProperty(PARAMETER_ABBREVIATIONS, true, false, false);
  ...
  public Map<String, String> getParameters() {
    ...
    parameters.put(
        ...
        GT._T(
            "A list of abbreviations that generate false positives when placed before {0}",
            "&lt;ref&gt;"));
